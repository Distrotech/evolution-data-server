				...
				GError             **error)
{
	 ...
	 */
	success = e_book_sqlite_lock_updates (priv->ebsql, FALSE, error);
	...
	if (success) {
		success = e_book_sqlite_unlock_updates (priv->ebsql, TRUE, error);

	} else {
		GError *local_error = NULL;

		/* Rollback transaction */
		if (!e_book_sqlite_unlock_updates (priv->ebsql, FALSE, &local_error)) {
